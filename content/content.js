...
if (window.GrokLoopInjected) {
                ...
                try {
                    ...
                    if (!seg.inputImage && index > 0) {
                        ...
                        if (prevSeg.videoUrl) {
                            ...
                            // Fallback if we didn't extract proactively
                    ...
                    // Extract frame NOW while video is hot/valid, for the NEXT segment
    ...
    // --- Helper Functions (dataURItoBlob, blobToBase64, uploadImageToGrok, sendPromptToGrok, waitForVideoResponse, extractLastFrame) ---
    // (Keeping implementations identical but defined inside the block to avoid global scope pollution if possible, 
    // or just hoisting them is fine for `var`/function, but strictly we want to avoid redeclaration errors)

    // We can leave them as function declarations at the bottom of this block

    // ... [Original Helper Functions go here] ...
    // Since I need to replace the WHOLE file to wrap it, I will paste the helpers back in.
    ...
    async function uploadImageToGrok(input) {
        ...
        if (!fileInput) {
            ...
            if (closeButton) {
                ...
                // We click back because otherwise we simply CANNOT upload.
                // The input is hidden/removed in this view.
                closeButton.click();
    ...
    async function sendPromptToGrok(text) {
        ...
        for (let attempt = 0; attempt < maxRetries; attempt++) {
            ...
            // 2. Loose selectors (grab all candidates and filter)
            ...
            if (attempt === 1 && visibleCandidates.length > 0) {
                console.log('Visible input candidates:', visibleCandidates);
            }
            ...
            // 2b. Fallback: Any visible textarea/contenteditable
            if (!inputArea && visibleCandidates.length > 0) {
                // Avoid search bars if possible
                inputArea = visibleCandidates.find(el => {
                    const placeholder = (el.placeholder || el.getAttribute('aria-placeholder') || '').toLowerCase();
                    return !placeholder.includes('search');
                });
                if (inputArea) console.log('Using loose fallback input.');
            }

            if (inputArea) break;

            // Debug logging to help identify why it's missing
            if (attempt === 5) {
                console.log('Visible inputs debug:', visibleCandidates);
            }
        ...
        // Final fallback: If there is ONLY ONE visible text input on the screen, assume it is the one.
        if (!inputArea) {
            ...
            if (visibleInputs.length === 1) {
                ...
                console.log('Falling back to the only visible input found.');
            }
        }
        ...
        if (inputArea.tagName === 'DIV' && inputArea.isContentEditable) {
            // ProseMirror/TipTap specific handling
            // We can't just set innerHTML or textContent because it breaks the editor state often.
            // Best way is to select all and insert text via command.
            ...
            // Allow event loop to catch up
            await new Promise(r => setTimeout(r, 100));

            // Use deprecated but reliable execCommand which editors intercept
            const success = document.execCommand('insertText', false, text);

            // If execCommand didn't work (returned false), try direct manipulation + events
            if (!success || inputArea.textContent.trim().length === 0) {
                console.log('execCommand failed, trying direct TextNode injection...');
                inputArea.innerHTML = `<p>${text}</p>`;
                inputArea.dispatchEvent(new Event('input', { bubbles: true }));
                inputArea.dispatchEvent(new Event('change', { bubbles: true }));
            }
        } else {
            const nativeDesc = Object.getOwnPropertyDescriptor(window[inputArea.tagName === 'TEXTAREA' ? 'HTMLTextAreaElement' : 'HTMLInputElement'].prototype, "value");
            if (nativeDesc && nativeDesc.set) {
                nativeDesc.set.call(inputArea, text);
            } else {
        ...
        await new Promise(r => setTimeout(r, 500));
        if (!inputArea.value && !inputArea.textContent.includes(text)) {
            inputArea.value = text;
